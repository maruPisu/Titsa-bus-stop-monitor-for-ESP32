...
void fetchData(const char* url) {
  if (WiFi.status() == WL_CONNECTED) {
    ...
    if (httpCode > 0) {
      ...
      if (!error) {
        if (doc["success"] == true && doc["lineas"].size() > 0) {
          String txt = "Parada " + doc["parada"]["id"].as<String>();
          write(txt.c_str(), false);
          ...
          for (JsonObject linea : lineas) {
            ...
            // Aggiorna solo se valido
            if (nuevo_tiempo > 0) {
              tiempo_actual = nuevo_tiempo;
              lastUpdate = millis();

              String resultado = "Linea " + linea_id + ": " + String(nuevo_tiempo) + " min";
              write(resultado.c_str(), false);  // .c_str() converts String to const char*
            }
            
          }

        } else {
          Serial.println("Nessun dato disponibile");
        }
      } else {
